...
| Fable II (GOTY) | [himdo/Fable-2-Recomp](https://github.com/himdo/Fable-2-Recomp) | Windows | Static recompilation of the X360 version via ReXGlue SDK v0.10.0. Built largely with a local LLM (Qwen) as an experiment, then refined by hand; stated as completable but not yet 100%'d. Requires a rip of the Fable II GOTY (USA/EU) disc (`default.xex`, `data/`, `nxeart/`, `$SystemUpdate/`). Vulkan backend option, keyboard→gamepad mapping, mouse look, and an uncapped-FPS launcher. |
...
| Super Mario Eclipse | [gamebanana.com](https://gamebanana.com/mods/536309) | Super Mario Sunshine (GameCube) — Dolphin or real hardware | Total-conversion mod by JoshuaMK02, Tempo and team (~5 years in development). New story, open-world-style structure, 120 new Shines, new worlds, cutscenes, and an original OST. Playable as Mario, Luigi, or Il Piantissimo. Requires a Super Mario Sunshine ISO. Not a recompilation — runs on the original engine. A co-op variant also exists. |
